...
This directory contains five projects. Two of them implement a TCP client and server using the Boost.Asio library. Another two provide equivalent implementations using the Qt Networking library. The fifth project is an alternative design for the Qt-based server, making a total of five projects.

All clients and servers are non-blocking, meaning they can run alongside your application's main event loop. The following sections compare the Boost.Asio and Qt implementations in terms of both design and performance.
...
In my implementation, I use the free functions `async_read` and `async_write`. These functions rely heavily on a callback-based interface, which naturally pushes the callback approach into any code that interacts with them.
...
Each client that connects to the server should have its own session object stored somewhere in memory. A session typically contains the accepted socket and the buffers used for communication. To manage these sessions, the server assigns a unique ID to each client and stores the corresponding session in a map data member.
