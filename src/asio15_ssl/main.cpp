#include <asio15_ssl_pb/Person.pb.h>
...
int server() {
  ...
  ssl_ctx.set_password_callback(
      []([[maybe_unused]] size_t max_len,
         [[maybe_unused]] ssl::context::password_purpose purpose) {
        return "pass";
      });
  ssl_ctx.use_certificate_chain_file("server.crt");
  ssl_ctx.use_private_key_file("server.key", ssl::context::pem);
  ssl_ctx.use_tmp_dh_file("dhparams.pem");
  ssl::stream<ip::tcp::socket> ssl_stream{ioc, ssl_ctx};
  ...
  try {
    ...
    read(ssl_stream, buffer(&len, sizeof(len)));
    read(ssl_stream, data_buf, transfer_exactly(len));
...
int client() {
  ...
  try {
    ...
    write(ssl_stream, buffer(&len, sizeof(len)));
    write(ssl_stream, data_buf);
  } catch (const boost::system::system_error &e) {
    std::cerr << "Error occured! Error code = " << e.code()
              << ". Message: " << e.what();
    return e.code().value();
